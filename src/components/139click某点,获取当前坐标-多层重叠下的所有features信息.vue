...
published in CSDN * @First published time: 2022-09-30 */

<template>
  <div class="container">
    <h3>vue+openlayers:单击某点，获取多层重叠位置下的所有features信息</h3>
    <p>大剑师兰特, 还是大剑师兰特</p>
    <h4>
      <el-button type="primary" size="mini" @click="showPolygon(city1)"
        >显示多边形1</el-button
      >
      <el-button type="primary" size="mini" @click="showPolygon(city2)"
        >显示多边形2</el-button
      >
      <el-button type="primary" size="mini" @click="clearLayer()"
        >清除图层</el-button
      >
    </h4>
  ...
  methods: {
    clickPoint() {
      ...
      this.map.on("click", (evt) => {
        ...
        const pixel = this.map.getEventPixel(evt.originalEvent); //Returns the map pixel position for a browser event relative to the viewport.
        ...
        if (feas && feas.length == 1) {
          ...
          this.overlayer.setPosition(evt.coordinate);
